...
> **在途事项（2026-09-18 深夜后，0.2.27 待批）**：**有**——0.2.27 已出包未发（`apps/desktop/dist/Fundet-Setup-0.2.27-x64.exe`），内容=欢迎页撤用量盘（4990079）+ 组件板可下钻/零溢出（a904d4d）+ 灵动岛驻会话头深度融合（5839761）+ 知识库面板卡片化（722e524）+ 卡片材质与零位移批（157140f）+ **主页弹层交互批**（本次 commit）。用户指令「先不要发版，一直调整到满意为止」——**满意说发再走发版流**。**主页交互模型（用户拍板：主页绝不能因交互变化，点击弹窗、窗内可下拉）**：DwsWidgets 双模式——popover（欢迎页）= 板面纯静态（定高 258 卡+卡内滚动，无原地展开），条目点击弹 fixed 浮层（锚卡下方，预算式定高 min(440,视口余量)，下方放不下翻卡上方，两边挤则顶天立地；浮层内全量列表+手风琴+内滚，Esc/外点/X 关，交给智能体钮随关）；inline（灵动岛）= 原地手风琴不变。实现：各卡内容抽成 Body 组件（mode board/detail 复用，board 切 3 条+溢出行）。**卡片材质**：token `--card-shadow(-hover)` + `.fundet-surface`（亮=顶缘 1px 白高光+暖黑双层影，暗=纯黑双层影；首版 50 分提档后过），组件卡/KB 实卡共用，幽灵卡无材质。KB 卡片形态：2 列网格、icon chip 头+hero「N 份文档」+meta 行、Reveal 展开内嵌（导入/笔记/快照/重试/发丝文档行）、虚线幽灵新建卡、交错入场。功能全量保留，typecheck 0 + 151/151 + 打包 CDP 冒烟 16/16（弹层 8：板 rect 不变/浮层开/可滚/视口内/内展开外框稳/Esc 关；板+KB 8）。**方法论教训**：视觉模型幻觉本会话第三例——评审断言与 DOM 实测矛盾时**必须 DOM 仲裁**（美感判断可用，事实判断不可信）；CDP 复拍两态单脚本一次完成；mjs 正则别写 `\\d`；**种子目录别放脚本**（launcher 自 wipe 会吃掉同目录脚本）；PowerShell 内联过 Git Bash 时 `$_` 被吞——一律 .ps1 文件；EBUSY 无进程占用时=Defender 扫描锁新签名 exe，重跑即过；**React 早退 return null 后不能再挂 useEffect**（DwsWidgets 曾犯：null→ready 钩子数变化会崩）；**弹层只设 max-h 不定高则内容仍会撑高外框**——要「窗内滚动」必须显式定高。
