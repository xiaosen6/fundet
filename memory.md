...
- **操作栏/输入行对齐（2026-09-18，未发版）**：①输入卡下方**上下文用量环移除**（用户裁决不要；上下文用量仍在 assistant 操作栏 tokens tooltip 可见）；②消息「更多」菜单对齐 Cindy：min-w-184px/h-8 行/删除前分隔线/菜单按 align 双侧对齐，**Rewind 收进菜单**（Undo2「回滚」，仅 user 侧），无「复制消息链接」（Cindy 云端，红线）；③按钮双序：user=[时间][复制][分享][分叉][编辑][更多]，assistant=[复制][分享][分叉][更多][时间][tokens]。
