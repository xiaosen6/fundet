...
> **在途事项（2026-09-20，0.2.29 待发）**：**有**——0.2.28 后新修三件（用户实测反馈）：①**灵动岛弹层内条目点不开（0.2.28 回归）**——弹层交互重构时 ListCtx 用单一 mode 字段糅合了「列表密度」和「点击行为」，inline 模式（灵动岛）的条目点击去开一个只在 popover 模式渲染的浮层=点空；修复=拆正交双开关 `sliced`（前3+溢出行）×`expandable`（原地展开 vs 开浮层），岛=sliced+expandable、欢迎页板=sliced+!expandable、详情浮层=!sliced+expandable。②**岛挤占 Canvas 钮（实测重叠 -16px）**——结构性错位：Canvas 固定钮占 right-[138px] 起 40px 宽（实际左缘 innerWidth-178），而头部内容 pr 只留 150px，标题/岛天然压进按钮热区（当年 138+12 的算式漏了按钮自身宽度）；修复=头部 pr-[150px]→pr-[186px] + 岛根 mr-3，实测间距 -16px→20px。③**MCP 服务器移回设置页（用户拍板「不用显示在主页栏」）**——侧栏能力入口剩 IM 机器人/钉钉工作台/技能/知识库四钮（SidebarPanelId 去掉 'mcp'，Puzzle 图标清理），SettingsPage 新增 mcp tab（位置：模型供应商之后，McpPanel 组件原样复用）。验证：岛内展开 PASS、主页弹层回归 8/8、侧栏 4 钮无 MCP、设置 MCP tab 渲染正常（blender 列表在）、typecheck+162/162。④**输入框自动增高修复**——autoResize 一直存在（写 style.height）但 textarea 挂 flex-1（basis 0），flex 主轴算法无视显式高度→长文本只出滚动条不出增高；修复=去 flex-1（高度纯内容驱动 200px 封顶）+ useEffect[value] 同步程序化改值（草稿恢复/引用插入/清空）。实测 1行24px→5行105px→14行封顶200px出滚动→删减缩回。另：内网模型 fundet-mini 上下文已由用户侧提到 262K，400 问题服务端解决。**首条消息慢四联修**：①快照移出发送关键路径——`enqueueSnapshot` 后台串行队列（每会话一条链防同仓并发 git 撞 index.lock，fire-and-forget 零等待）；②workDir=主目录跳过快照 + EXCLUDES 补 AppData/Library/.cache/.gradle/.venv/venv/__pycache__；③陈旧 index.lock 自愈（>15s 清，救活曾被锁死的回滚）；④MCP 桥全并行拉起 + stdio 预热 initialize 限时 15s。实测 dev 冷首条 32.0s→11.7s（剩余=pi 启动 6.4s【打字期间完成】+模型首请求）。**组件失败态**：errOf 人话错误透传（dws error.message→stderr 尾行→退出码）；profile 失败分流（401/token/授权→提示重登；其余保登录态+旧数据+四卡角标等重试，不再误判未登录致整板消失）；角标可点重试。
