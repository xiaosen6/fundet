...
> **在途事项（2026-09-19，0.2.27 待批）**：**有**——0.2.27 已出包未发（`apps/desktop/dist/Fundet-Setup-0.2.27-x64.exe`），内容=欢迎页撤用量盘（4990079）+ 组件板可下钻/零溢出（a904d4d）+ 灵动岛驻会话头深度融合（5839761）+ 知识库面板卡片化（722e524）+ 卡片材质与零位移批（157140f）+ 主页弹层交互批（e0da4b6）+ 主页自适应无滚动批（28da1ac）+ **messages_fts 建表 bug 修复**（本次 commit，**存量装机硬阻塞：0.2.22~0.2.26 未打开过会话搜索的库首次发消息即 `SqliteError: no such table: messages_fts`**）。根因：ensure 只挂在 searchSessions（搜索入口），伴生写 upsertMessageFts 裸 INSERT；开发机/种子库都打开过搜索所以表在，冒烟全绿假阴性。修复：抽 `db/messages-fts.ts` 纯模块（无 electron 依赖、db 注入），upsert 写路径**自建表**（ensureMessagesFts 幂等：CREATE IF NOT EXISTS + 空表时回填存量；去模块级 ftsReady 标志——多库注入会串）。测试 +5（156/156：无表库 upsert 不炸/清残留/rowid 复用 REPLACE/回填幂等/bigram MATCH——断言库文本是分词串非原文）。端到端实证（修复包+种子库 DROP 表后 send）：发送成功、表自动回来 66 行（回填 65+新 1）、「张章」可 MATCH。**教训三条**：① 之前答复用户「表是自动建的」只实证了 KB 表——messages_fts 是另一条链，用户真机报错是对的；② **行为断言会被乐观回显骗**（旧包发送失败但 DOM 显示消息、toast 4 秒后消失）——存证类验证以**杀进程后物理查 db** 为裁决；③ PowerShell 里 node -e 内联 `COUNT(*)` 的 `*`/引号会被吞，复杂内联一律走 Bash 或脚本文件。**主页永不滚动架构**：欢迎列 h-full + 板区 flex-1 + 卡行 1fr（660 封顶/150 地板，窗口最小高 640 放得下），真窗口缩放 800/700/640 全档零溢出。**主页交互模型**：DwsWidgets 双模式 popover（板静态+弹浮层定高内滚）/inline（灵动岛）。**卡片材质**：--card-shadow(-hover)+.fundet-surface（亮=白高光+暖黑双层影/暗=纯黑双层影）。KB 卡片化形态如前。其余方法论教训（视觉模型幻觉以 DOM 仲裁/种子目录勿放脚本/$_ 吞噬/EBUSY=Defender 重跑/早退 return 后禁挂 useEffect/浮层须显式定高/缩窗用渲染层 resizeTo）见 git 历史各 commit message。
