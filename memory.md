...
- **侧栏左上能力入口抽屉化（2026-09-14，用户要求）**：IM 机器人/技能/MCP 服务器/**知识库**（新增第四个）四按钮常驻侧栏顶部，点击滑出**独立抽屉面板**（复用设置页 Panel 组件，portal 渲染、Esc/遮罩/× 关闭），不再跳转设置页；设置页同步摘除对应四块（SettingsTab 收敛为 general/providers/automation/usage/search）。组件：`components/sidebar/SidebarPanelDrawer.tsx`。
