...
| 版本 | 日期 | 要点 |
| --- | --- | --- |
| 0.3.0 | 09-21 | **品牌换新（保留 Fundet 名）+ Cindy 全对齐 + 自动化**：红色头盔小宇航员 logo（圆形透明角全套）；新会话空消息 = Cindy 首页复刻（lockup 44px 圆+44px 字 / 输入卡 790×110 占主区 77% / 建议卡池 10 条[换一批/不再显示]，逐像素测量对齐）；**亮色主题换 Cindy 原值**；**自动化（定时例行任务，Cindy 侧栏第二钮全搬）**：宿主层调度（非插件沙箱）——六触发类型（每小时/每天/工作日/每周/每月/cron 五段）+ 指令，到点起隔离会话（auto- 前缀不进侧栏）跑默认模型，运行历史（queued/running/success/failed）+ 暂停/立即运行/漏跑补一次；drizzle 幂等双表、零依赖 automation-schedule.ts（7 cron 单测）、181/181。曾当日改「未灵」后按用户拍板回退 Fundet。同版本含 SkillHub 集市（0.2.30 并入） |
