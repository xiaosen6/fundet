...
| 版本 | 日期 | 要点 |
| --- | --- | --- |
| 0.2.30 | 09-20 | **SkillHub 技能集市（M1+M2）**：设置→技能双 tab（已安装/发现）——发现页接 api.skillhub.cn（匿名免令牌，list 10min TTL）：搜索 300ms 防抖+序号防串、四排序（下载/趋势/星标/评分）、分类 chips、卡片流（**彩色图标走 skillhub-icon:// 主进程代理：域白名单+1MB 上限+魔数嗅探+磁盘缓存，CSP 不开 https**）、展开详情（版本徽标/作者验证/三源安全审计/更新日志）；一键安装管线（清单校验[≤50 文件≤5MB 防穿越]+4 并发下载+逐文件 sha256+临时目录原子落位 `~/.agents/skills/<slug>/`+skillhub.json 元数据），重装备份替换，已装 tab 比对显更新。176/176 测试，dev+打包双端真 API 实测（30 卡/搜索/详情/安装 1.5s 物理验证，图标 24/28 渲染） |
...
> **在途事项（2026-09-20，0.2.30 未发）**：**SkillHub 技能集市 M1+M2 已实现待用户验收**（本地已提交，**未 tag 未发版**）——`shared/skillhub.ts` 类型；`main/host/skillhub.ts`（api.skillhub.cn 匿名客户端 + 安装管线：清单校验→4 并发下载→逐文件 sha256→临时目录原子 rename 到 `~/.agents/skills/<slug>/`→写 skillhub.json 元数据；重装走备份替换；≤50 文件 ≤5MB 防穿越）+ 零依赖 `skill-frontmatter.ts`（node --test 下 agent-core 是 TS 源不可 import，剥出验证器）；单测（fixture=真 API 捕获）+ 图标 4 组；IPC 四通道；SkillsPanel 双 tab。实测：dev+打包（db-only 种子 9343）双端真 API 30 卡/搜索/详情/安装 1.5s 物理验证+清场。**图标功能（用户要求加技能图片）**：根因是 CSP `img-src` 不放行 https → 10/10 有 icon_url 却全回退 Zap；修法=**`skillhub-icon://` 主进程代理协议**（skillhub-icon-protocol.ts：域白名单[cloudcache.tencent-cloud.com/.myqcloud.com/skillhub.cn] + ≤1MB + 魔数嗅探 content-type + userData/skillhub-icons 磁盘缓存 sha256(url) + 并发去重；CSP 只加这一个 scheme，聊天远程图管控不变）；渲染层 `skillhubIconProxyUrl()` 纯改写（shared）；图标 32→44px 圆角方块 + ring-white/10 描边（黑底图标防融入卡片）；**展开详情不放第二个大图标**（视觉评审：与头部图标重复+结构重排突兀，展开只向下延伸）。dev 实测 24/28 渲染（其余 icon_url 失效回退 Zap 属正常）。**skillhub.cn 事实**：api.skillhub.cn/api/v1 匿名无频限（search/skills/{slug}/files/file），文件走腾讯 COS CDN 302；排序白名单 downloads/trending/stars/score；真包普遍带 `_meta.json`（下划线开头）——FILE_PATH_RE 必须放开首下划线（初版这里挡死安装）。**M2 裁剪**：合集（skillsets 端点已验证可用）未做 UI，M3 缓冲服务器/版本历史回滚/发布未动。**测试纪律（新教训）**：打包版渲染进程无 `require`，E2E 目录数检查用 CDP 拿不到 fs——物理磁盘检查为最终仲裁；**冒烟种子必须 db-only（fundet.db* 共 1.2MB）**——整份拷贝 %APPDATA%\Fundet 会带 ~8GB 检查点快照，历史 6 份把 C 盘写爆到 400MB（2026-09-20 用户实报，46.4GB 已清，launch-dev/sh-pack-launch 均已改最小种子）。
