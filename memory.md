...
> **在途事项（2026-09-17）**：**优化批已全部落地、未发版**（commits 63ce971/03f0c0f/d00ef68，建议随 dd5ffe6 上游安全批一起发 0.2.21；**发版说明需提**：#4518 后「完全放行」档会话里模型写自己运行时目录会弹一次审批卡，属安全设计内）。功能八项：①**思考档位 UI**（EffortSelector——DB/IPC/thinkingLevelMap 数据链早已就绪，此前唯独没 UI；权限 chip 旁，推理模型才显示）；②**会话搜索**（`db/session-search.ts`：标题 LIKE + 正文 FTS5 复用知识库 CJK bigram 分词；messages_fts 幂等回填 + insertMessage 同步 upsert，rowid 复用靠 OR REPLACE 清残留；侧栏搜索框 200ms 防抖，Esc 清空）；③**错误分类自动重试**（`lib/errorRetry.ts` 分类器限流/过载/网络瞬断 → 倒计时重发最多 2 次；SessionSendInput 加 `retry` 标记让 main 跳过重复插库；done/新发送/abort/连接重置均取消）；④**开机自启**（设置→通用开关；dev 拒写 Run 键；`build/installer.nsh` 卸载清登录项——electron-builder nsis.include 已挂）；⑤**分享卡片 DOM 光栅化**（html-to-image 新 devDep + `clipboard:write-png` IPC 带魔数/尺寸校验；治 capturePage 最小化截空图）；⑥**本轮产出文件卡**（write/edit 目标回溯派生，chip 点开 Canvas）；⑦**Vertex/Azure BYOK 预设**（endpoint 模板 + host 识别）；⑧@ 引用 placeholder 提示。工程：updater GitLab 死代码全清（brand.updaterFeed/令牌 IPC 四件套/设置页令牌 UI，更新单线 GitHub app-update.yml）；发版冒烟脚本化 `tools/smoke-installer.mjs`（0.2.20 包实测 PASS）。验证：typecheck 全过、desktop 106 + agent-core 875 全绿、dev:win + CDP 核验（搜索框/设置登录卡渲染、searchSessions/loginItem IPC 实测、坏 PNG 正确拒绝）。**剩余待做（§7 大件，需单独排期）**：checkpoint/文件回滚、消息排队、Goal 托管、Ollama。
>
