...
| 版本 | 日期 | 要点 |
| --- | --- | --- |
| 0.2.30 | 09-20 | **SkillHub 技能集市（M1+M2）**：设置→技能双 tab（已安装/发现）——发现页接 api.skillhub.cn（匿名免令牌，list 10min TTL）：**全库搜索（服务端真参数 q，中英文；keyword 被服务端忽略曾致搜索=默认榜，实测修复）**300ms 防抖+序号防串、四排序（下载/趋势/星标/评分）、分类 chips、纯文字卡片流（图标做完实测通过后用户拍板不要，整体拆除）+重复 slug 去重、展开详情（版本徽标/作者验证/三源安全审计/更新日志）；一键安装管线（清单校验[≤50 文件≤5MB 防穿越]+4 并发下载+逐文件 sha256+临时目录原子落位 `~/.agents/skills/<slug>/`+skillhub.json 元数据），重装备份替换，已装 tab 比对显更新。174/174 测试，dev+打包双端真 API 实测（30 卡/搜索/详情/安装 1.5s 物理验证） |
