...
> **在途事项（2026-09-24，0.3.13 发版被阻）**：**代码全部就绪**（commit f085da8，209/209，冒烟过），**卡在打包机楔子**——electron-builder 稳定挂死在「searching for node modules」（单核 60-97% CPU 空转，不超时不报错）。**已排除**：仓库内容（回退 0.3.12 package.json 照挂）、路径（C:\fb 全新镜像挂）、node_modules 状态（pnpm install 全新重建挂）、文件系统速度（readdir/realpath 毫秒级）、符号链接环（无）、pm 模式（npm/pnpm 双模式同挂）——**结论：机器级状态问题（过滤器驱动/句柄楔化），昨 17:38 同机同仓构建正常**。**起病相关**：13:33 我跑打包版冒烟 → Fundet 死后 **crashpad_handler 孤儿进程锁住 dist\win-unpacked\resources\app.asar**（杀 Fundet 不够，crashpad 会留！）→ 后续一切构建异常。**修复路径：重启电脑后重跑 `pnpm build && pnpm dist:win` 即可**（其余发版步骤全部就绪：draft 先建+独立进程上传那套）。**新坑入账**：①打包版冒烟后必须查杀 crashpad_handler 残留再打包；②Restart Manager API（C:\temp\fundet-dev-tools\FileLockFinder.cs）可定位文件占用者（比猜快）；③robocopy /XD dist 会排掉**所有层级**同名目录（误杀 vendored dist）且不保符号链接——镜像 pnpm 仓必须 rm node_modules + `pnpm install --frozen-lockfile` 重建；④pnpm install 新装会跳过 better-sqlite3 构建脚本，需 `pnpm rebuild better-sqlite3` 补。
