...
> **在途事项（2026-09-20，0.2.28 待发）**：**有**——0.2.28 已出包未发（`apps/desktop/dist/Fundet-Setup-0.2.28-x64.exe`；版本从 0.2.27 升位防与已发布 Release 撞号），内容=**首条消息慢四联修 + 组件失败态可诊断化**。①快照移出发送关键路径——`enqueueSnapshot` 后台串行队列（每会话一条链防同仓并发 git 撞 index.lock，fire-and-forget 零等待；register.ts 只入队不 await）；②workDir=用户主目录跳过快照（回滚整个 home 不现实）+ EXCLUDES 补 AppData/Library/.cache/.gradle/.venv/venv/__pycache__；③陈旧 index.lock 自愈（>15s 视为上次超时残物，快照/回滚前清——救活曾被锁死的回滚功能）；④MCP 桥全并行拉起（曾串行逐块 await）+ stdio 预热 initialize 限时 15s（曾共用 600s 请求超时，zombie server 可无限卡死会话装配）。**实测 dev 冷首条 32.0s→11.7s**（剩余=pi 子进程启动+bridge 连 6.4s【真实用户打字期间即完成】+模型首请求 ~4.7s）。⑤**组件「刷新失败」批（外部用户报"加载失败"）**：errOf 人话错误透传（dws stdout error.message 优先[doskey 免疫]→stderr 尾行→退出码兜底，120 字截断）——此前所有人所有原因只显「退出码 N」；profile 命令失败不再误判"还没登录"（曾整板消失）：报错含 401/token/授权 → disabled 提示"登录态失效，重新登录"，否则保登录态+旧数据、四卡挂"钉钉连接失败：原因"角标等下轮重试；失败角标从纯文本变可点重试钮（tooltip 带完整原因）。单测 +3（errOf 三态），162/162；dev+打包成功态回归：4 卡 0 角标、布局/弹层 13/13。**测量坑（本轮新增）**：DOM 计时标记除不能进用户消息外还**必须每轮随机**——复用种子库时旧会话可能含同串 token 假秒命中；种子库默认模型可能指向内网网关（Fundet@10.7.0.95:16667，离线时 Connection error）——打包测延迟前先核对 providers 默认指向。
