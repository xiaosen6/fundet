...
| 版本 | 日期 | 要点 |
| --- | --- | --- |
| 0.3.0 | 09-21 | **品牌重塑：未灵 Weiling**——新 logo（红色头盔小宇航员吉祥物，全套 icon.ico 七尺寸/favicon/logo-fundet 512）+ 产品展示名全链改「未灵」（窗口标题/Splash/侧栏/输入占位/系统提示词身份「AI 灵伴助手」）；打包 productName=Weiling（ASCII 安装目录）、快捷方式「未灵」、安装包 Weiling-Setup；**技术标识全保留**：appId com.fundet.app、GitHub 仓 xiaosen6/fundet（更新通道不断）、userData 钉死 %APPDATA%\Fundet（main app.setName+显式拼 Fundet，存量数据零迁移）、HKCU Run 键/卸载清理对齐。命名由来：未来互联之「未」+灵动精灵，英文谐音 willing。同版本含 SkillHub 集市（见 0.2.30 在途并入此发） |
