...
> **在途事项（2026-09-20，0.2.28 待发）**：**有**——0.2.28 已出包未发（`apps/desktop/dist/Fundet-Setup-0.2.28-x64.exe`；版本从 0.2.27 升位防与已发布 Release 撞号），内容=**首条消息慢四联修（用户拍板「全做」）**：①快照移出发送关键路径——`enqueueSnapshot` 后台串行队列（每会话一条链防同仓并发 git 撞 index.lock，fire-and-forget 零等待；register.ts 只入队不 await）；②workDir=用户主目录跳过快照（回滚整个 home 不现实）+ EXCLUDES 补 AppData/Library/.cache/.gradle/.venv/venv/__pycache__；③陈旧 index.lock 自愈（>15s 视为上次超时残物，快照/回滚前清——救活曾被锁死的回滚功能）；④MCP 桥全并行拉起（曾串行逐块 await）+ stdio 预热 initialize 限时 15s（曾共用 600s 请求超时，zombie server 可无限卡死会话装配）。**实测 dev 冷首条 32.0s→11.7s**（剩余=pi 子进程启动+bridge 连 6.4s【真实用户打字期间即完成】+模型首请求 ~4.7s）；单测 +3（home 跳过/锁自愈/队列），159/159；打包回归：消息正常落库、无 no such table 复发。**测量坑（本轮新增）**：DOM 计时标记除不能进用户消息外还**必须每轮随机**——复用种子库时旧会话可能含同串 token 假秒命中；种子库默认模型可能指向内网网关（Fundet@10.7.0.95:16667，离线时 Connection error）——打包测延迟前先核对 providers 默认指向。
