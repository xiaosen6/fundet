...
> **在途事项（2026-09-18 深夜后，0.2.27 待批）**：**有**——0.2.27 已出包未发（`apps/desktop/dist/Fundet-Setup-0.2.27-x64.exe`），内容=欢迎页撤用量盘（4990079）+ 组件板可下钻/零溢出（a904d4d）+ 灵动岛驻会话头深度融合（5839761）+ 知识库面板卡片化（722e524）+ **卡片材质与零位移批**（本次 commit）。用户指令「先不要发版，一直调整到满意为止」——**满意说发再走发版流**。**零位移架构**：组件卡定高 `h-[258px]` + 卡体 `overflow-y-auto`——条目展开只撑卡内滚动，板面/页面/滚动位置恒定（展开前后 grid rect 逐字节相同，dev+打包两轮 DOM 实测）；日程序列对齐待办/未读的 3+溢出行（原来 6 场全渲染会常态滚动）；UnreadDetail 去嵌套滚动（双层滚动手感差）。**卡片材质**：token `--card-shadow(-hover)` + `.fundet-surface` 工具类（亮色=顶缘 1px 白高光内描边 + 暖黑双层影[近距贴身+大范围弥散]，暗色=纯黑双层影无白线；首版强度不足被评审打回，提档后过），组件卡/KB 实卡共用，幽灵卡刻意无材质（虚实对比）。KB 卡片形态：2 列网格（sm），卡=icon chip 头+名称+空库徽标+hero「N 份文档」大数字+meta 行（片段/建于/点击管理）；Reveal 展开内嵌卡内（导入 pill 行/进度条/笔记编辑/URL 快照/结果重试/发丝文档行 divide-board/60+悬停浮现编辑删除）；网格尾**虚线幽灵新建卡**（self-start 展开邻卡时不拉伸、单卡时右半不空旷、点击聚焦顶部输入框）；交错入场 60ms。功能全量保留（CRUD/导入/笔记/快照/进度/重试），typecheck 0 + 151/151 + 打包 CDP 冒烟 8/8×2 轮。**方法论教训**：视觉模型幻觉本会话第三例——KB 收起态评审断言「无幽灵卡、hero 旧排版」，DOM 实测全在（dashedCount=1、hero="2份文档"）——**截图评审结论必须 DOM 仲裁再信**（美感判断可用，事实判断不可信）；CDP 复拍两态要单脚本一次完成（两次调用中间再点侧栏=关面板，第二张必拍错）；mjs 断言正则别写 `\\d`（字面反斜杠假失败）；**dev/userData 种子目录里别放脚本**（kb-design.ps1 自wipe整目录把同目录脚本全吃了，脚本移居 fundet-dev-tools）；PowerShell 内联命令过 Git Bash 时 `$_` 会被 bash 吞——一律写 .ps1 文件；EBUSY 新形态=无进程占用时是 Defender 扫描锁刚签名的 exe，重跑即过（本次撞两次、第三次过）。
