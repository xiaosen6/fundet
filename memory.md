...
| 版本 | 日期 | 要点 |
| --- | --- | --- |
| 0.3.6 | 09-22 | **热补 6：知识库引用圈重载丢失**——用户实报：回复里的【n】小圈（可点溯源）重开会话后变纯文本。根因：**tool_result_full 事件（带工具结果全文）从未落库**（persistEvent 的 switch 没这个 case，只有 tool_use/tool_result 摘要），重载后 knowledgeSourcesFor 拿不到 resultText→无来源→rehype 不转圈。修两处：①persistEvent 补 tool_result_full 落库（{kind,data}，toolUseId 过滤）；②sessionStore DB 恢复路径把 fullText 挂回对应 tool item 的 resultText（此前只标 done）。**存量会话救不回**（库里本来就没存全文），新回复起永久保留。验证方法论：零模型消耗——手工往种子库插 tool_result_full+带【1】assistant 合成一轮，reload 后 5 个可点圈+溯源面板 PASS；**引用圈 DOM 口径**：sup.kb-cite 渲染时被 components.sup 替换成 button（title=查看来源【n】），querySelectorAll(.kb-cite) 永远 0，探针要查 button[title^=查看来源]（同坑两次） || 0.3.6 | 09-22 | **热补 6：知识库引用圈重载丢失**——用户实报：回复里的【n】小圈（可点溯源）重开会话后变纯文本。根因：**tool_result_full 事件（带工具结果全文）从未落库**（persistEvent 的 switch 没这个 case，只有 tool_use/tool_result 摘要），重载后 knowledgeSourcesFor 拿不到 resultText→无来源→rehype 不转圈。修两处：①persistEvent 补 tool_result_full 落库（{kind,data}，toolUseId 过滤）；②sessionStore DB 恢复路径把 fullText 挂回对应 tool item 的 resultText（此前只标 done）。**存量会话救不回**（库里本来就没存全文），新回复起永久保留。验证方法论：零模型消耗——手工往种子库插 tool_result_full+带【1】assistant 合成一轮，reload 后 5 个可点圈+溯源面板 PASS；**引用圈 DOM 口径**：sup.kb-cite 渲染时被 components.sup 替换成 button（title=查看来源【n】），querySelectorAll(.kb-cite) 永远 0，探针要查 button[title^=查看来源]（同坑两次） |；**重装体验**——「重装中」黑盒（用户实报一直转）：本机实测官方脚本 47s 装完，慢网络上限 10 分钟超时（runCommand kill+报错不挂死），期间原零反馈。修：点击立即 notice 预期（约 1 分钟/最长 10 分钟）+ 新增「用 GitHub 源」备选按钮（Gitee 失败时换源）；**安装进度条**——runCommandStreaming（spawn 增量 onOutput）→ DWS_INSTALL_PROGRESS push → DwsPanel 订阅：时间型进度条（0-60s 线性 5→72%，60-600s 缓爬至 95%，完成/失败收敛）+ 已进行 X 分 X 秒 + 实时输出尾巴 6 行滚动（下载类无精确百分比，时间型+真输出是最诚实的进度）
