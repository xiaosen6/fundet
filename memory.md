...
> **在途事项（2026-09-19，0.2.27 待批）**：**有**——0.2.27 已出包未发（`apps/desktop/dist/Fundet-Setup-0.2.27-x64.exe`），内容=欢迎页撤用量盘（4990079）+ 组件板可下钻/零溢出（a904d4d）+ 灵动岛驻会话头深度融合（5839761）+ 知识库面板卡片化（722e524）+ 卡片材质与零位移批（157140f）+ 主页弹层交互批（e0da4b6）+ **主页自适应无滚动批**（本次 commit）。用户指令「先不要发版，一直调整到满意为止」——**满意说发再走发版流**。**主页永不滚动架构（用户两轮反馈拍板：右侧绝不能比侧栏长）**：欢迎列 `h-full` 撑满 + 板区 `flex-1` + 网格行 `sm:grid-rows-[repeat(2,minmax(0,1fr))]` 弹性分配（卡 660 封顶防高屏过空、150 地板——app 窗口最小高 640 也放得下）；品牌/引导卡 shrink-0，间距收紧（gap-2.5/py-2/引导卡 py-3.5）。实测（渲染层 `window.resizeTo` 真缩窗，Emulation.setDeviceMetricsOverride 在 Electron 不生效、Browser.setWindowBounds 需 browser 级 WS 被拒）：800h→卡 223、700h→173、640h→150，全部 scrollH==clientH 零溢出。**主页交互模型**：DwsWidgets 双模式——popover（欢迎页）板面纯静态，条目点击弹 fixed 浮层（锚卡下方、预算式定高 min(440,视口余量)、下方不够翻卡上方、窗内全量+手风琴+滚动、Esc/外点/X 关）；inline（灵动岛）原地手风琴不变。各卡内容抽 Body 组件（mode board/detail 复用）。**卡片材质**：token `--card-shadow(-hover)` + `.fundet-surface`（亮=顶缘 1px 白高光+暖黑双层影，暗=纯黑双层影），组件卡/KB 实卡共用，幽灵卡无材质。KB 卡片形态：2 列网格、icon chip 头+hero「N 份文档」+meta 行、Reveal 展开内嵌、虚线幽灵新建卡、交错入场。typecheck 0 + 151/151 + 打包 CDP 冒烟 13/13（无滚动三档 5 + 弹层 8）+ dev 弹层回归 8/8，亮暗截图评审无硬伤。**方法论教训**：视觉模型幻觉本会话第三例——评审断言与 DOM 实测矛盾时**必须 DOM 仲裁**（美感判断可用，事实判断不可信）；CDP 复拍两态单脚本一次完成；mjs 正则别写 `\\d`；**种子目录别放脚本**（launcher 自 wipe 会吃掉同目录脚本，脚本居 fundet-dev-tools）；PowerShell 内联过 Git Bash `$_` 被吞——一律 .ps1 文件；EBUSY 无进程占用时=Defender 扫描锁新签名 exe，重跑即过；**React 早退 return null 后不能再挂 useEffect**（钩子数随 snapshot 变化会崩）；**浮层要「窗内滚动」必须显式定高**（只设 max-h 内容仍撑高外框）；**Electron 里缩窗验证用渲染层 window.resizeTo**（有效），Emulation override 无效、Browser.setWindowBounds 走 page WS 会 undefined。
