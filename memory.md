...
> 最后更新：2026-09-22（0.3.6）。给任何接手的人/AI：**先通读本文再动手**——版本历史（§3.5）按时间记录每个版本的决策与教训，尾部「在途事项」段记录最新状态；再读 README.md（用户向）。
>
...
| 版本 | 日期 | 要点 |
| --- | --- | --- |
| 0.3.14（待发） | 09-25 | **①技能安装护栏放行大包（用户实报「>50 报错」）**——真凶是安装校验 MAX_FILES=50（搜索侧本无限制、固定 30 条）：官方 dingtalk-misc 实测 **122 文件/1.5MB** 被「文件数超上限」拒装；上限校准 files 50→**300**、体积 5MB→**20MB**（防穿越/SKILL.md 校验不动；注意技能包不能「截断文件」——截了就残废，用户口径的截断按放行实现）。**②安装慢第一刀：asar 死重清除**——解剖发现 app.asar **189MB/14,362 文件**塞满渲染层依赖全量包（mermaid 系 113MB+katex 系 14MB，vite 早已打包、asar 里纯副本，连 __tests__/.eslintignore 都在）；六个纯渲染依赖挪 devDependencies（@tanstack/react-virtual/katex/mermaid/rehype-katex/remark-cjk-friendly/remark-math；**qrcode 主进程在用必须留**）；安装包 **288.6→258.3MB（-30MB）**、asar 189→53MB、文件数 14,362→5,190（用户机解压+Defender 扫描量同比减）。**③打包确定性修复：`win.signAndEditExecutable: false`**——四连 EBUSY 根因=eb26 的 signtool 逐个锁 exe 改版本资源（rcedit 替代品，我们没证书这步纯装饰），与 extraResources 拷贝重试竞态，慢日确定性撞死；关掉后打包 10min→2.7min 且无轮盘赌。**遗留提案（安装慢的下载侧大头，待拍板）**：④Git Bash 改按需下载（再 -86MB，dws 镜像同款链路）；⑤**Gitee 镜像更新源**（updater generic provider 双源回落，CN 用户下载 200KB/s→MB/s 级——dws-mirror 同套资产镜像可复用）。209/209，打包版冒烟全过（loopback 种子） |
...
> **在途事项（2026-09-24，0.3.13 发）**：**无**——重启后楔子全清，0.3.13 已发 GitHub Release：tag=commit d18d001（含版本提交 f085da8，merge-base 验证），三资产齐（资产端点全 uploaded + gh-proxy 镜像直连双复核：exe 206 断点/blockmap 200/latest.yml sha512 与本地逐字节吻合——**直连 GitHub 下载当晚不稳，核验资产可用 gh-proxy 镜像**），非 draft；安装包备份 `D:\Fundet-Setup-0.3.13-x64.exe`（275MB）。打包版冒烟 PASS：资产齐（含 EBUSY 重跑后的 cua dll）/loopback provider 种子（**models 列必须注对象数组 [{id}] 非字符串数组——字符串会让 buildPiNativeProviders 抛异常 → nativeResolveFailed**）/预热+回收闭环（pi 起+灭）/缩放锚点（1600 宽窗最大化 1.5→1.875）。发版流程走 draft 先建 + Start-Process 独立进程上传（~5 分钟）+ edit 发布，全程无卡。
