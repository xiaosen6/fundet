...
| 版本 | 日期 | 要点 |
| --- | --- | --- |
| 0.3.0 | 09-21 | **品牌换新（保留 Fundet 名）+ Cindy 全对齐**：红色头盔小宇航员 logo（**圆形透明角全套图标**）；新会话空消息 = Cindy 首页复刻（40px 圆 logo+24px 名 lockup + 引导建议卡池 10 条[换一批/不再显示]，点击预填 prompt）；**亮色主题换 Cindy 原值**（冷灰白系 asar 抄录，暗色本就是 Cindy 原值）；Esc 分层。曾当日改「未灵」后按用户拍板回退 Fundet（名/打包全回，技术标识从未动过——userData/GitHub 仓零迁移）。同版本含 SkillHub 集市（0.2.30 并入此发） |
