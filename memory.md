...
| 版本 | 日期 | 要点 |
| --- | --- | --- |
| 0.3.10（待发） | 09-23 | **钉钉知识库勾选（用户设计：选源交给用户，不靠语义理解）**——KnowledgeChip 面板顶部新增「钉钉知识库」开关（binding.dingtalk，JSON 向后兼容）；勾了注入 dingtalk_kb_search MCP 工具（handler 调 dws aisearch enterprise --queries，60s 超时，解析 errorCode/登录失效提示）；MCP server 改动态工具组合（handlers:{search?,list?,dingtalk?} 按绑定出现，tools/list 只列勾了的）——勾谁谁的工具在，消歧由用户显式完成；零依赖 dingtalk-format.ts（formatDingtalkResults 纯函数，样本=dws v1.0.62 实跑脱敏）；189/189（+钉钉解析 5 组+动态组合 1 组）。**架构注意**：tool.ts import store 会拉 Electron 链（node-test 挂）——纯函数独立文件 |
